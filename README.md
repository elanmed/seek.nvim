...
- When invoking `seek()`, the next two chars are recorded:
    ...
    - Else, `seek()` searches for substring matches of the recorded chars 
        - If there's one match, the cursor is set to the location of the first typed char and the jumplist is updated
        - Else, an extmark label is set immediately after the match
